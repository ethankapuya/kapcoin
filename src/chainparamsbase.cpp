...
{
    // kapcoin RPC + P2P ports — distinct from Litecoin/Bitcoin defaults
    if (chain == CBaseChainParams::MAIN) {
        return MakeUnique<CBaseChainParams>("", 9554, 9555);
    } else if (chain == CBaseChainParams::TESTNET) {
        return MakeUnique<CBaseChainParams>("testnet4", 19554, 19555);
    } else if (chain == CBaseChainParams::SIGNET) {
        return MakeUnique<CBaseChainParams>("signet", 39554, 39555);
    } else if (chain == CBaseChainParams::REGTEST) {
        return MakeUnique<CBaseChainParams>("regtest", 19557, 19556);
    }
